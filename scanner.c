...
static lox_token_type determine_identifier_type() {
    switch (scanner.start[0]) {
        ...
        case 'c': return check_for_keyword(1, 4, "lass", TOKEN_CLASS);
        case 'e': return check_for_keyword(1, 3, "lse", TOKEN_ELSE);
        case 'f':
            if (scanner.current - scanner.start > 1) {
                switch (scanner.start[1]) {
                    case 'a': return check_for_keyword(2, 3, "lse", TOKEN_FALSE);
                    case 'o': return check_for_keyword(2, 1, "r", TOKEN_FOR);
                    case 'u': return check_for_keyword(2, 1, "n", TOKEN_FUN);
                }
            }
            break;
        ...
        case 's': return check_for_keyword(1, 4, "uper", TOKEN_SUPER);
        case 't':
            if (scanner.current - scanner.start > 1) {
                switch (scanner.start[1]) {
                    case 'h': check_for_keyword(2, 2, "is", TOKEN_THIS);
                    case 'r': check_for_keyword(2, 2, "ue", TOKEN_TRUE);
                }
            }
            break;
        case 'v': return check_for_keyword(1, 2, "ar", TOKEN_VAR);
